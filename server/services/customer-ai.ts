...
const SYSTEM_PROMPT = `أنت مساعد ذكي في هذا المتجر. لست شخصاً حقيقياً — أنت ذكاء اصطناعي يساعد الزبون.

تتحدث بالعربية الفصحى الطبيعية والمفهومة ولا تستخدم أي لهجات عامية أو كلمات أجنبية.

إذا كان الزبون يراسلك للمرة الأولى ترحب به بلطف واختصار. إذا كانت المحادثة مستمرة تدخل في صلب الإجابة مباشرة دون تكرار التحية.

لا تخمن معلومات من رأسك. إذا كان المنتج موجوداً في القائمة أعلاه استخدم معلوماته بدقة. إذا لم تجد المنتج أو لم تكن متأكداً من معلومة، قل بصراحة "هذه المعلومة غير متوفرة لدي". لا تقل "المنتج غير متوفر" إلا إذا كنت متأكداً.
...
عندما تكتمل البيانات الخمسة (المنتج، الكمية، الاسم، الهاتف، الولاية)، لا تسأل الزبون "هل تريد إتمام الطلب؟" — اكتب أمر الطلب فوراً:
...
إذا طلب الزبون منتجاً له ألوان في القائمة، استخدم الألوان المذكورة حصراً ولا تخترع ألواناً من عندك.`;
...
function buildUserPrompt(ctx: SlimContext, search: string, orderText: string, phone: string | null, phoneFromMsg: boolean, msg: string, history: GeminiContent[] = [], factsSummary: string = ''): string {
  ...
  if (isFirstMessage) {
    p += `\n[حالة الحوار: هذه بداية المحادثة. افتتح الرد بتحية ودية ترحيبية مختصرة. لا تذكر معلومات التوصيل أو الدفع أو سياسة المتجر إلا إذا سأل عنها الزبون صراحةً.]\n`;
  } else {
  ...
  if (search) {
    ...
    if (simpleGreetings.includes(trimmedMsg) || trimmedMsg === 'ه') {
      p += `\n[حالة السلع: تحية بسيطة. رد بتحية قصيرة واسأل كيف يمكنك المساعدة.]\n`;
    } else {
      p += `\n[حالة السلع: لم يتطابق كلام الزبون مع أي منتج محدد في المتجر. يمنع تخمين اسم منتج أو إنشاء طلب جديد ما لم يذكر الزبون اسم المنتج صراحةً في رسالته الحالية. إذا كان الزبون يحتاج pomocy مع منتج أو تسليم أو لديه سؤال، يمكنك طرح أسئلة توضيحية بلطف.]\n`;
    }
  }
  ...
  p += `[تنبيه]: المعلومات أعلاه (التوصيل والسياسة) للإطلاع فقط. لا تذكرها في ردك إلا إذا سأل عنها الزبون صراحةً. إذا كان مجرد ترحيب أو سؤال بسيط، اختصر ردك بشدة.\n`;
