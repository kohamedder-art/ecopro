// HHD Express Courier Service
// HHD Express runs on the Ecotrack platform: https://hhdexpress.ecotrack.dz
...
const BASE_URL = 'https://hhdexpress.ecotrack.dz/api/v1';
...
export class HhdService implements CourierService {
    ...
    try {
      ...
      if (rawPhone.length < 9) {
        return {
          success: false,
          tracking_number: '',
          error: `Phone number "${shipment.customer_phone}" is too short for HHD Express (minimum 9 digits)`,
        };
      }
      ...
      if (!response.ok) {
        return {
          success: false,
          tracking_number: '',
          error: data?.message || `HHD Express API error ${response.status}`,
        };
      }
      ...
      console.error('[HHD Express] createShipment error:', error);
  ...
  async testCredentials(apiKey: string, _accountId?: string): Promise<import('../courier-service').CourierTestResult> {
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        return { success: false, message: 'Invalid HHD Express API token — access denied' };
      }
      if (!response.ok) {
        const data = await this.readJson(response);
        return { success: false, message: data?.message || `HHD Express API error ${response.status}` };
      }
      return { success: true, message: 'HHD Express credentials verified successfully' };
    } catch (error: any) {
      return { success: false, message: error?.message || 'Failed to connect to HHD Express API' };
    }
  }
}
