...
function StoreSwitcher() {
  ...
  return (
    <div className="px-3 py-2.5 border-b border-slate-100 dark:border-slate-700/50">
      <div className="text-[9px] font-bold uppercase tracking-[0.15em] text-slate-400 dark:text-slate-500 mb-2 px-1">
        المتجر النشط
      </div>
      <div className="space-y-0.5">
        {stores.map(store => {
          ...
          return (
            ...
            >
              <span
                className={`w-2.5 h-2.5 rounded-full shrink-0 ring-2 ring-offset-1 ring-offset-white dark:ring-offset-slate-900 ${
                  isActive ? 'ring-indigo-500/40' : 'ring-transparent'
                }`}
                style={{ backgroundColor: store.primary_color || '#a0876a' }}
              />
              <span className="truncate">{store.store_name}</span>
              {isActive && (
                <span className="mr-auto text-[10px] text-indigo-400 dark:text-indigo-500 font-bold">
                  ●
                </span>
              )}
            </button>
      ...
      {!showCreate ? (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 mt-1.5 px-2.5 py-2 rounded-xl text-xs font-semibold text-indigo-500 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors border border-dashed border-indigo-300 dark:border-indigo-500/30"
        >
          <span className="text-base leading-none">+</span>
          متجر جديد
        </button>
      ) : (
        <div className="mt-1.5 p-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
          <input
            type="text"
            value={newStoreName}
            onChange={e => setNewStoreName(e.target.value)}
            placeholder="اسم المتجر الجديد"
            className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="flex-1 px-3 py-1.5 rounded-lg bg-indigo-500 text-white text-xs font-bold hover:bg-indigo-600 disabled:opacity-50 transition-colors"
            >
              {creating ? '...' : 'إنشاء'}
            </button>
            <button
