...
interface OrderSuccessConnectProps {
  storeSlug: string;
  accentColor?: string;
...
export default function OrderSuccessConnect({
  storeSlug,
  accentColor = '#2563eb',
  orderId,
  customerPhone,
  ...
  useEffect(() => {
    if (!storeSlug) return;
    fetch(`/api/storefront/${encodeURIComponent(storeSlug)}/contact-channels`)
  ...
  const buildUrl = (ch: ContactChannel) => {
    ...
    if (ch.platform === 'whatsapp' && orderId) {
      const msg = encodeURIComponent(`مرحباً، أريد متابعة طلبي رقم #${orderId}`);
  ...
  return (
    <div className="mt-4 pt-4 border-t border-current/10" dir="rtl">
      <p className="text-sm font-bold mb-4 text-center">
        📲 تابع طلبك وتلقَّ التحديثات عبر
      </p>
      <div className="flex flex-wrap justify-center gap-3">
        {messagingChannels.map((ch) => (
          <a
            ...
            className="inline-flex flex-col items-center gap-2 px-5 py-3 rounded-2xl text-white font-bold transition-all hover:scale-110 hover:shadow-xl active:scale-95 shadow-lg border-2"
            style={{ 
              backgroundColor: PLATFORM_COLORS[ch.platform] || accentColor,
              borderColor: PLATFORM_COLORS[ch.platform] || accentColor,
            }}
          >
            <div className="w-8 h-8 flex items-center justify-center">
              {PLATFORM_ICONS[ch.platform]}
            </div>
            <span className="text-xs font-semibold">{PLATFORM_LABELS[ch.platform] || ch.label}</span>
          </a>
        ))}
      </div>
