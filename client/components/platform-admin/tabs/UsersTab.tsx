...
export default function UsersTab({ users, onRefresh, loading }: Props) {
  ...
  function getRoleBadge(user: User) {
    ...
    if (role === 'admin') return <Badge className="bg-purple-500/20 text-purple-300 border border-purple-500/30 text-[10px]">Admin</Badge>;
    return <Badge className="bg-blue-500/20 text-blue-300 border border-blue-500/30 text-[10px]">Client</Badge>;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="bg-slate-800/60 border border-slate-700/50 rounded-lg text-xs text-slate-300 px-3 py-2 outline-none"
        >
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-700/30">
            {filtered.slice(0, 50).map(user => (
              <div key={user.id} className="group hover:bg-slate-900/30 transition-colors">
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {getRoleBadge(user)}
                      {user.is_super && <Badge className="bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[10px]">Super</Badge>}
                    </div>
                ...
                {expandedUser === user.id && (
                  <div className="px-3 pb-3 ml-12 space-y-1">
                    ...
                    <div className="text-xs text-slate-500">
                      ID: {user.id} · Role: {user.role || user.user_type} · Joined: {new Date(user.created_at).toLocaleString()}
                    </div>
                  </div>
                )}
              </div>
