  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button onClick={onReload} disabled={loading} className="bg-slate-700 hover:bg-slate-600 text-white text-xs h-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            ...
            className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl text-white placeholder-slate-500 text-sm pl-10 pr-4 py-2 focus:border-red-500/50 outline-none"
          />
        </div>
        <div className="flex gap-1">
          <button onClick={() => setView('active')} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${view === 'active' ? 'bg-red-600 text-white' : 'bg-slate-800/60 text-slate-400'}`}>
            Active ({activeCount})
          </button>
          <button onClick={() => setView('all')} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${view === 'all' ? 'bg-red-600 text-white' : 'bg-slate-800/60 text-slate-400'}`}>
            All ({errors.length})
          </button>
        </div>
        <div className="flex gap-1">
          {(['all', 'client', 'server'] as const).map(s => (
            <button key={s} onClick={() => setSource(s)}
              className={`px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1 ${source === s ? 'bg-slate-600 text-white' : 'bg-slate-800/60 text-slate-400'}`}
            >
        ...
        <button onClick={() => setGrouped(!grouped)}
          className={`px-2.5 py-1.5 rounded-lg text-xs font-medium ${grouped ? 'bg-purple-600 text-white' : 'bg-slate-800/60 text-slate-400'}`}
        >
          {grouped ? 'Grouped' : 'Ungrouped'}
        </button>
        <select value={days} onChange={e => setDays(Number(e.target.value))}
          className="bg-slate-800/60 border border-slate-700/50 rounded-lg text-xs text-slate-300 px-2 py-1.5"
        >
      ...
      {/* Error List */}
      <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/40 overflow-hidden shadow-lg">
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-emerald-300 text-sm font-medium">No errors found</p>
            <p className="text-slate-500 text-xs mt-1">Platform is running clean</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-700/30">
            {displayErrors.slice(0, 50).map(({ ev, count, latest }, i) => {
              ...
              return (
                <div key={i} className="p-3 hover:bg-slate-900/30 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white font-medium truncate">{msg.slice(0, 120)}</p>
                      {where && <p className="text-xs text-slate-500 mt-0.5 font-mono truncate">{where}</p>}
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-[10px] text-slate-500">
                          {new Date(latest).toLocaleString()}
                        </span>
                        {ev.source && (
                          <Badge className="bg-slate-700/50 text-slate-400 text-[10px]">{ev.source}</Badge>
                        )}
                      </div>
                    </div>
