  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white/60 dark:bg-slate-800/40 rounded-2xl border border-slate-700/40 p-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
          {filtered.map(p => {
            ...
            return (
            <div key={p.id} className={`group bg-white/60 dark:bg-slate-800/40 rounded-xl border ${p.flagged ? 'border-red-500/40' : 'border-slate-700/40'} overflow-hidden shadow-md hover:shadow-lg transition-shadow`}>
              {/* Image — falls back to video thumbnail */}
              <div className="aspect-square bg-slate-900/60 relative overflow-hidden">
                {thumb ? (
                  <div className="w-full h-full relative">
                    <img src={thumb} alt={p.title} width={200} height={200} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                    {isVideoFallback && (
        ...
        <div className="bg-white/60 dark:bg-slate-800/40 rounded-2xl border border-slate-700/40 overflow-hidden">
            ...
            {filtered.map(p => {
              ...
              <div key={p.id} className="group flex items-center gap-3 p-3 hover:bg-gray-50/30 dark:bg-slate-900/30 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-slate-900/60 overflow-hidden flex-shrink-0 relative">
                  {thumb ? (
                    <img src={thumb} alt="" width={40} height={40} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                  ) : (
