...
export default function StoresTab({ stores, loading }: Props) {
  ...
  function getStatusBadge(status?: string) {
    switch (status) {
      case 'active': return <Badge className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px]">{t('platformAdmin.stores.active')}</Badge>;
      case 'trial': return <Badge className="bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[10px]">{t('platformAdmin.stores.trial')}</Badge>;
      case 'expired': case 'locked': return <Badge className="bg-red-500/20 text-red-300 border border-red-500/30 text-[10px]">{t('platformAdmin.stores.expired')}</Badge>;
      default: return <Badge className="bg-slate-500/20 text-gray-500 dark:text-slate-400 border border-slate-500/30 text-[10px]">{status || 'Unknown'}</Badge>;
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white/60 dark:bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/40 p-4">
          <p className="text-xs text-gray-500 dark:text-slate-400">{t('platformAdmin.stores.totalStores')}</p>
          <p className="text-2xl font-black text-gray-900 dark:text-white mt-1">{stores.length}</p>
        </div>
        <div className="bg-emerald-500/10 backdrop-blur-xl rounded-2xl border border-emerald-500/30 p-4">
          <p className="text-xs text-emerald-300">{t('platformAdmin.stores.active')}</p>
        ...
        <div className="bg-amber-500/10 backdrop-blur-xl rounded-2xl border border-amber-500/30 p-4">
          <p className="text-xs text-amber-300">{t('platformAdmin.stores.trial')}</p>
        ...
        <div className="bg-red-500/10 backdrop-blur-xl rounded-2xl border border-red-500/30 p-4">
          <p className="text-xs text-red-300">{t('platformAdmin.stores.expired')}</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder={t('platformAdmin.stores.searchPlaceholder')}
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full bg-white/60 dark:bg-slate-800/60 border border-gray-200 dark:border-slate-700/50 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 text-sm ps-10 pe-4 py-2.5 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 outline-none transition-all"
          />
        </div>
          ...
          {(['all', 'active', 'trial', 'expired'] as const).map(f => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                statusFilter === f ? 'bg-blue-600 text-white' : 'bg-white/60 dark:bg-slate-800/60 text-gray-500 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-700'
              }`}
            >
              {f === 'all' ? t('platformAdmin.stores.all') : f === 'active' ? t('platformAdmin.stores.active') : f === 'trial' ? t('platformAdmin.stores.trial') : t('platformAdmin.stores.expired')}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white/60 dark:bg-slate-800/40 rounded-2xl border border-slate-700/40 py-12 text-center">
          ...
          <p className="text-gray-500 dark:text-slate-500 text-sm">{t('platformAdmin.stores.noStores')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map(store => (
            <div key={store.id} className="group bg-white/60 dark:bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-slate-700/40 p-4 hover:border-slate-600/60 transition-all shadow-lg hover:shadow-xl">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-sm font-bold text-gray-900 dark:text-white truncate">{store.store_name || t('platformAdmin.stores.unnamed')}</h4>
                      <p className="text-[10px] text-gray-500 dark:text-slate-500 truncate">{store.store_slug}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-1.5 text-xs">
                ...
                {store.paid_until && (
                  <div className="flex items-center justify-between text-gray-500 dark:text-slate-400">
                    <span>{t('platformAdmin.stores.paidUntil')}</span>
                    <span className="text-gray-600 dark:text-slate-300">{new Date(store.paid_until).toLocaleDateString()}</span>
                  </div>
                )}
              </div>

              <div className="mt-3 pt-2 border-t border-gray-200/60 dark:border-slate-700/30 flex items-center gap-2">
                <a
                ...
                >
                  ...
                  {t('platformAdmin.stores.visitStore')}
                </a>
              </div>
            </div>
          ))}
