...
export default function ArtisanTemplate(props: TemplateProps) {
  ...
  const showTrustSection = (settings as any)?.artisan_show_trust !== false;
      ...
      <section className="m-6 p-8 rounded-[2.5rem] relative overflow-visible" style={{ backgroundColor: '#3e2c23', color: bgColor }} data-edit-path="trust-section">
        {canManage && (
            <div className="absolute -top-3 left-4 flex items-center gap-1 bg-violet-600 text-white text-xs px-2 py-1 rounded-full shadow-lg z-10">
                <button
                    onClick={() => window.parent.postMessage({ type: 'TEMPLATE_UPDATE_SETTING', key: 'artisan_show_trust', value: !showTrustSection }, '*')}
        ...
        {canManage && !showTrustSection && (
            <span className="text-white/50 text-xs">🛡️ Trust section hidden</span>
        )}
      </section>
      )}
