...
export default function LuminaTemplate({ settings, products, canManage, storeSlug }: TemplateProps) {
  ...
  const showAnnouncement = settings?.lumina_show_announcement !== false;
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800" style={{ backgroundColor: settings?.template_bg_color || '#f8fafc' }} dir="rtl">
      ...
      <div className="text-white text-center py-2.5 text-sm font-bold flex justify-center items-center gap-2 relative overflow-visible" style={{ backgroundColor: accentColor }} data-edit-path="announcement-bar">
        {canManage && (
            <div className="absolute -top-3 left-4 flex items-center gap-1 bg-violet-600 text-white text-xs px-2 py-1 rounded-full shadow-lg z-10">
                <button
                    onClick={() => window.parent.postMessage({ type: 'TEMPLATE_UPDATE_SETTING', key: 'lumina_show_announcement', value: !showAnnouncement }, '*')}
                    className="flex items-center gap-1 font-bold"
                >
                    {showAnnouncement ? <><Eye className="w-3 h-3"/> إخفاء</> : <><EyeOff className="w-3 h-3"/> إظهار</>}
                </button>
            </div>
        )}
        {showAnnouncement && (
        ...
        {canManage && !showAnnouncement && (
            <span className="text-white/70 text-[10px]">📢 Announcement hidden</span>
        )}
      </div>
      )}
