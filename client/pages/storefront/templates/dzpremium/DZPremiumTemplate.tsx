...
export default function DZPremiumTemplate({ settings, products, canManage, storeSlug }: TemplateProps) {
    ...
    const showBadge = settings?.dzp_show_badge !== false;
    const showBenefits = settings?.dzp_show_benefits !== false;
        ...
        <div style={{ fontFamily: "'Tajawal', sans-serif", backgroundColor: settings?.template_bg_color || undefined }} className="pb-32 md:pb-0 bg-white text-gray-900" dir="rtl">
                    ...
                    <div className="mb-4 flex justify-center relative overflow-visible" data-edit-path="badge">
                        {canManage && (
                            <div className="absolute -top-3 left-4 flex items-center gap-1 bg-violet-600 text-white text-xs px-2 py-1 rounded-full shadow-lg z-10">
                                <button
                                    onClick={() => window.parent.postMessage({ type: 'TEMPLATE_UPDATE_SETTING', key: 'dzp_show_badge', value: !showBadge }, '*')}
                                    className="flex items-center gap-1 font-bold"
                                >
                                    {showBadge ? <><Eye className="w-3 h-3"/> إخفاء</> : <><EyeOff className="w-3 h-3"/> إظهار</>}
                                </button>
                            </div>
                        )}
                        {showBadge && (
                        ...
                        {canManage && !showBadge && (
                            <span className="text-gray-400 text-xs">🏷️ Badge hidden</span>
                        )}
                    </div>
                    )}
            ...
            <section className="bg-gray-50 py-12 px-4 relative overflow-visible" data-edit-path="benefits">
                {canManage && (
                    <div className="absolute -top-3 left-4 flex items-center gap-1 bg-violet-600 text-white text-xs px-2 py-1 rounded-full shadow-lg z-10">
                        <button
                            onClick={() => window.parent.postMessage({ type: 'TEMPLATE_UPDATE_SETTING', key: 'dzp_show_benefits', value: !showBenefits }, '*')}
                            className="flex items-center gap-1 font-bold"
                        >
                            {showBenefits ? <><Eye className="w-3 h-3"/> إخفاء</> : <><EyeOff className="w-3 h-3"/> إظهار</>}
                        </button>
                    </div>
                )}
                {showBenefits && (
                ...
                {canManage && !showBenefits && (
                    <div className="text-center py-4 text-gray-400 text-xs">🛡️ Benefits hidden</div>
                )}
            </section>
            )}
