      ...
      <div className="gallery-img-wrap flex flex-col h-full">
        <div className="gallery-img-main relative w-full aspect-square overflow-hidden shrink-0" style={{ backgroundColor: surfaceMuted }}
          ...
          {videoEmbed && showVideo ? (
            ...
            <img src={imgs[idx] || imgs[0]} alt="" className="w-full h-full object-cover transition-all duration-300" onClick={() => { const p = activeProduct; const imgs2 = p?.images?.filter(Boolean) || []; setZoomState({ images: imgs2.length ? imgs2 : [imgs[idx] || imgs[0]], idx }); }} />
          ) : (
