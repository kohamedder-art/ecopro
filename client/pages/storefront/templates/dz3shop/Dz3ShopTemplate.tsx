              ...
              <div className="w-full lg:w-1/2 flex flex-col gap-3 lg:self-stretch">
                ...
                <div className="rounded-xl overflow-hidden shadow-sm relative aspect-[4/5] lg:aspect-auto lg:flex-1 lg:max-h-[70vh]" style={{ backgroundColor: surfaceColor }}>
                  ...
                  {detailImages.length > 1 && (
                    <>
                      <button onClick={e => { e.stopPropagation(); const t = detailImages.length + (videoEmbed?1:0); const c = showVideo ? 0 : (videoEmbed ? activeImageIndex + 1 : activeImageIndex); const n = (c + 1) % t; if (n === 0 && videoEmbed) { setShowVideo(true); scrollCarouselTo(0); } else { setShowVideo(false); const ii = videoEmbed ? n - 1 : n; setActiveImageIndex(ii); scrollCarouselTo(n); } }}
                      ...
                      <button onClick={e => { e.stopPropagation(); const t = detailImages.length + (videoEmbed?1:0); const c = showVideo ? 0 : (videoEmbed ? activeImageIndex + 1 : activeImageIndex); const p = (c - 1 + t) % t; if (p === 0 && videoEmbed) { setShowVideo(true); scrollCarouselTo(0); } else { setShowVideo(false); const ii = videoEmbed ? p - 1 : p; setActiveImageIndex(ii); scrollCarouselTo(p); } }}
